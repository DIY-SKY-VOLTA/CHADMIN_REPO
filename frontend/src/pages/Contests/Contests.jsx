    ...
    <div className="h-full flex flex-col bg-neutral-50/30 dark:bg-[#0d0d0f]/20 selection:bg-neutral-200/50 dark:selection:bg-neutral-400/40">
      ...
      <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-neutral-200/50 dark:border-white/5 bg-white/40 dark:bg-[#121214]/40 backdrop-blur-sm">
        <div>
          <h1 className="text-base font-semibold text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
            <Trophy size={16} strokeWidth={1.5} className="text-neutral-500 dark:text-neutral-400" />
            Contest Manager
          </h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-md bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 hover:opacity-90 transition-all shadow-sm flex items-center gap-1.5 text-[11px] font-semibold"
          >
      ...
      <div className="shrink-0 px-6 py-3 flex flex-col md:flex-row gap-3 items-center justify-between border-b border-neutral-200/30 dark:border-white/[0.04]">
        <div className="w-full md:w-72 relative">
          <Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-500 dark:text-neutral-400" />
          <input
            type="text"
            value={search}
        ...
        <div className="w-full md:w-auto flex flex-wrap items-center gap-2 justify-end">
          ...
          <select
            ...
            className="max-w-[160px] px-2.5 py-1.5 bg-white dark:bg-[#151518] border border-neutral-200/60 dark:border-white/5 rounded-md text-[11px] font-medium text-neutral-600 dark:text-neutral-400 focus:outline-none"
          >
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-[#151518]/40 border border-neutral-200/40 dark:border-white/5 rounded-lg flex flex-col items-center justify-center py-20 shadow-sm">
            <Trophy size={28} strokeWidth={1.25} className="text-neutral-400 dark:text-neutral-600" />
            ...
            {!debouncedSearch && (
              <button
                onClick={() => navigate('/contests/new')}
                className="mt-4 px-3 py-1.5 rounded-md bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 hover:opacity-90 transition-all shadow-sm flex items-center gap-1.5 text-[11px] font-semibold"
              >
                <Plus size={12} strokeWidth={2.5} />
                Add Contest
              </button>
                  ...
                  >
                    ...
                    <div className="w-[30%] flex items-center gap-3 pr-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <span className="text-[9px] text-neutral-500 dark:text-neutral-400 truncate block font-mono">
                          /{contest.slug || '—'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="w-[13%] pr-3 truncate">
                      {contest.category ? (
                        <span className="px-1.5 py-0.5 rounded-md bg-neutral-100 dark:bg-neutral-800/50 text-[9px] font-medium text-neutral-600 dark:text-neutral-400 border border-neutral-200/50 dark:border-white/5 truncate inline-block max-w-full">
                          {contest.category}
                        </span>
                      ) : (
                    ...
                    <div className="w-[10%] pr-3">
                      <span className={`px-1.5 py-0.5 rounded-md text-[9px] font-semibold border ${contest.type === 'hackathon'
                        ? 'bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-500/15'
                        : 'bg-neutral-100 dark:bg-neutral-800/50 text-neutral-600 dark:text-neutral-400 border-neutral-200/50 dark:border-white/5'}`}>
                    ...
                    <div className="w-[9%] pr-3">
                      <span className={`inline-flex px-1.5 py-0.5 rounded-md text-[9px] font-bold border ${statusCfg.bg} ${statusCfg.text} ${statusCfg.border}`}>
                        {statusCfg.label}
                      </span>
                    </div>

                    {/* Deadline */}
                    <div className="w-[13%] pr-3 flex items-center gap-1.5 text-[10px] text-neutral-600 dark:text-neutral-400">
                      <Calendar size={10} className="text-neutral-500 dark:text-neutral-400 shrink-0" />
                    ...
                    {/* Prize */}
                    <div className="w-[12%] pr-3 flex items-center gap-1.5 text-[10px] text-neutral-600 dark:text-neutral-400 min-w-0">
                      <DollarSign size={10} className="text-neutral-500 dark:text-neutral-400 shrink-0" />
                    ...
                    <div className="w-[13%] flex items-center justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                      <button
                        onClick={() => navigate(`/contests/${contest._id}/edit`)}
                        className="p-1.5 rounded-md hover:bg-neutral-100 dark:hover:bg-white/5 text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white transition-colors"
                        title="Edit contest"
                      >
                        <Pencil size={14} />
                      </button>
                      ...
                      {contest.link && (
                        <a
                          ...
                          className="p-1.5 rounded-md hover:bg-blue-500/10 text-neutral-600 dark:text-neutral-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors"
                      ...
                      {contest.archivedAt ? (
                        <button
                          ...
                          className="p-1.5 rounded-md hover:bg-emerald-500/10 text-neutral-600 dark:text-neutral-400 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors disabled:opacity-30"
                          title="Restore contest"
                        >
                          {busy === contest._id ? <Loader2 size={14} className="animate-spin" /> : <RotateCcw size={14} />}
                        </button>
                      ) : (
                        <button
                          onClick={() => handleArchive(contest)}
                          disabled={busy === contest._id}
                          className="p-1.5 rounded-md hover:bg-red-500/10 text-neutral-600 dark:text-neutral-400 hover:text-red-600 dark:hover:text-red-400 transition-colors disabled:opacity-30"
                          title="Delete contest"
                        >
                          {busy === contest._id ? <Loader2 size={14} className="animate-spin" /> : <Trash2 size={14} />}
                        </button>
                      )}
                    </div>
                  </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 px-4 py-6">
            <button
              ...
              disabled={page <= 1}
              className="p-1.5 rounded-md border border-neutral-200/60 dark:border-white/5 bg-white dark:bg-[#151518] text-neutral-600 dark:text-neutral-400 hover:text-neutral-900 dark:hover:text-white disabled:opacity-30 transition-all shadow-sm"
            >
              <ChevronLeft size={14} />
            </button>
