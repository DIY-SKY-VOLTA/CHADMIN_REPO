...
export default function CategoriesPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isCreating) {
        const res = await adminAPI.post('/categories', editForm);
        if (res.success) {
        ...
        const res = await adminAPI.put(`/categories/${editingId}`, editForm);
        if (res.success) {
          toast.success(res.message);
  ...
  const handleDelete = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await adminAPI.delete(`/categories/${deleteTarget._id}`);
      if (res.success) {
        toast.success(res.message);
  ...
  const handleDragEnd = async () => {
    ...
    try {
      await adminAPI.put('/categories/reorder/batch', {
  ...
  const totalPostCount = categories.reduce((sum, cat) => sum + (cat.postCount || 0), 0);
  ...
  return (
    <div className="h-full flex flex-col bg-neutral-50/30 dark:bg-[#0d0d0f]/20 selection:bg-neutral-200/50 dark:selection:bg-neutral-400/40">
      ...
      <div className="shrink-0 flex items-center justify-between px-6 py-4 border-b border-neutral-200/50 dark:border-white/5 bg-white/40 dark:bg-[#121214]/40 backdrop-blur-sm">
        <div>
          <h1 className="text-base font-semibold text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
            <Tag size={16} strokeWidth={1.5} className="text-neutral-400" />
            Post Category Manager
          </h1>
          <p className="text-[11px] text-neutral-400 dark:text-neutral-500 mt-0.5">
            {categories.length > 0
              ? `Manage ${categories.length} content categories`
              : 'Add taxonomies and tags for articles'}
          </p>
        </div>
      ...
      <div className="shrink-0 p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-[#151518]/70 border border-neutral-200/40 dark:border-white/5 p-4 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-neutral-300 dark:hover:border-neutral-800 transition-all">
          <div className="space-y-1">
            <span className="text-[10px] font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">Active Associations</span>
            <p className="text-xl font-bold text-neutral-800 dark:text-neutral-100 leading-none">{totalPostCount}</p>
          </div>
        ...
        <div className="bg-white dark:bg-[#151518]/70 border border-neutral-200/40 dark:border-white/5 p-4 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-neutral-300 dark:hover:border-neutral-800 transition-all">
          ...
          <div className="w-8 h-8 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-600 dark:text-blue-550">
            <Tag size={15} strokeWidth={1.5} className="text-blue-600" />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-[#151518]/70 border border-neutral-200/40 dark:border-white/5 p-4 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex items-center justify-between group hover:border-neutral-300 dark:hover:border-neutral-800 transition-all">
          ...
          <div className="w-8 h-8 rounded-lg bg-amber-500/10 flex items-center justify-center text-amber-605 dark:text-amber-500">
      ...
      <div className="flex-1 overflow-y-auto px-6 pb-6">
        {isLoading ? (
          ...
          <div className="bg-white dark:bg-[#151518]/40 border border-neutral-200/40 dark:border-white/5 rounded-2xl flex flex-col items-center justify-center py-20 shadow-sm">
            <Tag size={28} strokeWidth={1.5} className="text-neutral-350 dark:text-neutral-600 mb-3" />
            <p className="text-xs font-semibold text-neutral-800 dark:text-neutral-300">No categories found</p>
            <p className="text-[11px] text-neutral-400 mt-1">Create categories to structure your blog content.</p>
            <button
              onClick={startCreating}
          ...
          <div className="bg-white dark:bg-[#151518]/70 border border-neutral-200/40 dark:border-white/5 rounded-xl overflow-hidden shadow-[0_1px_2px_rgba(0,0,0,0.01)]">
            <div className="min-w-full divide-y divide-neutral-200/50 dark:divide-white/5">
              ...
              <div className="bg-neutral-50/50 dark:bg-neutral-900/30 px-5 py-2.5 flex items-center text-[10px] font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider select-none">
                ...
                <div className="w-[32%]">Category Tag</div>
                <div className="w-[20%]">Slug / Path</div>
              ...
              <div className="divide-y divide-neutral-150 dark:divide-white/5">
                {categories.map((cat, idx) => (
                  ...
                  >
                    ...
                    <div className="w-[32%] pr-4 flex items-center gap-2">
                      <span
                    ...
                    {/* Slug */}
                    <div className="w-[20%] pr-4 truncate font-mono text-[10px] text-neutral-450 dark:text-neutral-500">
                      /{cat.slug || '—'}
                    </div>
                    ...
                    {/* Description */}
                    <div className="w-[20%] pr-4 truncate text-neutral-400 dark:text-neutral-550">
      ...
      {/* Slide-over Category Drawer Editor/Creator */}
      <AnimatePresence>
        {isDrawerOpen && (
          <>
            {/* Backdrop */}
            ...
            >
              ...
              <div className="shrink-0 p-4 border-b border-neutral-200/50 dark:border-white/5 flex items-center justify-between">
                <span className="text-[10px] font-semibold text-neutral-400 dark:text-neutral-500 uppercase tracking-wider">
                  {isCreating ? 'Create Taxonomy Category' : 'Modify Taxonomy Category'}
                </span>
                <button
                  onClick={() => setIsDrawerOpen(false)}
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      placeholder="Provide a short description of topics matching this tag..."
                      rows={3}
              ...
              <div className="shrink-0 p-4 border-t border-neutral-200/50 dark:border-white/5 bg-neutral-50/50 dark:bg-neutral-900/30 flex justify-between gap-3">
                ...
                >
                  ...
                  {isCreating ? 'Create Tag' : 'Save Details'}
                </button>
                <button
                  type="button"
                  onClick={() => setIsDrawerOpen(false)}
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-neutral-400 leading-relaxed">
                      Permanently remove <strong className="text-neutral-850 dark:text-white">{deleteTarget.name}</strong> from content taxonomies?
                    </p>
                  </div>
                </div>
                ...
                {deleteTarget.postCount > 0 && (
                  <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/10 text-amber-650 dark:text-amber-400 space-y-1">
                    ...
                    <p className="text-[10px] leading-relaxed">
                      There are {deleteTarget.postCount} active blog post(s) associated with this category.
                      You must reassign them to another category before deleting this taxonomy.
                    </p>
                  </div>
                )}
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    disabled={isSaving || (deleteTarget.postCount ?? 0) > 0}
